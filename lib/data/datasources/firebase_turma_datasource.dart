...
class FirebaseTurmaDatasourceImpl implements FirebaseTurmaDatasource {
  ...
  Future<List<Turma>> listarTurmasPorProfessor(String professorId) async {
    try {
      ...
      throw ServerException('Erro ao listar turmas do professor: ${e.toString()}');
    }
  }
  ...
  Future<bool> verificarCodigoTurma(String codigo) async {
    try {
      ...
      throw ServerException('Erro ao verificar código de turma: ${e.toString()}');
    }
  }
  ...
  Future<Turma> atualizarTurma(Turma turma) async {
    try {
      ...
      final turmaExiste = await _firestore.collection('turmas').doc(turma.id).get();
      ...
      // Atualizar turma
      await _firestore.collection('turmas').doc(turma.id).update(turma.toFirestore());
  ...
  Future<void> deletarTurma(String turmaId) async {
    try {
      // Verificar se a turma existe
      final turmaExiste = await _firestore.collection('turmas').doc(turmaId).get();
      if (!turmaExiste.exists) {
        throw TurmaNaoEncontradaException('ID não encontrado');
      }
  ...
  @override
  Future<Turma> adicionarAtividadeTurma(String turmaId, String atividadeId) async {
    try {
      // Buscar turma atual
      final turmaDoc = await _firestore.collection('turmas').doc(turmaId).get();
      ...
      throw ServerException('Erro ao adicionar atividade à turma: ${e.toString()}');
    }
  }
  
  @override
  Future<Turma> removerAtividadeTurma(String turmaId, String atividadeId) async {
    try {
      // Buscar turma atual
      final turmaDoc = await _firestore.collection('turmas').doc(turmaId).get();
      ...
      final newAtividades = turma.atividades.where((id) => id != atividadeId).toList();
      await _firestore.collection('turmas').doc(turmaId).update({
        'atividades': newAtividades,
      });
      ...
      throw ServerException('Erro ao remover atividade da turma: ${e.toString()}');
    }
  }
} 