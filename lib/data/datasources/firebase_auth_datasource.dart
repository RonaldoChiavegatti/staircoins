...
abstract class FirebaseAuthDatasource {
  ...
  Future<app_models.User> register(String name, String email, String password, app_models.UserType type);
...
class FirebaseAuthDatasourceImpl implements FirebaseAuthDatasource {
  ...
  Future<app_models.User?> getCurrentUser() async {
    try {
      ...
      throw FirebaseAuthException(code: e.code, message: e.message ?? 'Erro de autenticação');
    } catch (e) {
  ...
  @override
  Future<app_models.User> register(String name, String email, String password, app_models.UserType type) async {
    try {
      ...
      throw FirebaseAuthException(code: e.code, message: e.message ?? 'Erro ao registrar usuário');
    } catch (e) {
  ...
  Future<app_models.User> updateUser(app_models.User user) async {
    try {
      ...
      if (currentUser != null && currentUser.uid == user.id && currentUser.displayName != user.name) {
  ...
  Future<app_models.User> addTurmaToUser(String userId, String turmaId) async {
    try {
      ...
      throw ServerException('Erro ao adicionar turma ao usuário: ${e.toString()}');
    }
  }
  
  @override
  Future<app_models.User> removeTurmaFromUser(String userId, String turmaId) async {
    try {
      // Buscar usuário atual
      final userDoc = await _firestore.collection('users').doc(userId).get();
      ...
      throw ServerException('Erro ao remover turma do usuário: ${e.toString()}');
    }
  }
} 